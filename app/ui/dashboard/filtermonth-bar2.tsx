...
export default function FiltermonthBar2() {
    ...
    const handleChange = (id: string) => {
        ...
        if (id) {
            newSearchParams.set('agentId', id); // Cập nhật query param với agent=id
    ...
    return (
        <div className="flex flex-1 justify-between items-center bg-green-500 rounded-md py-[9px] px-2 text-black">
            <div className="flex flex-1 justify-start">
                <select
                    ...
                    {options?.map((option: Agent) => (
                        <option key={option.id} value={option.id}> {/* Đặt value là id */}
                            {option.agent}
                        </option>
                    ))}
                </select>
